...
int main(int argc, char *argv[])
{
	...
	int         nrOfLoops, bufferSize, bufferSizeLast, fileSize, i, isFinal;
	int         j = 0, ns = 0;
	clock_t     tstart, tend;
	double      cpuTime = 0.0;
	...
	for (i = 0; i < nrOfLoops; ++i)
	{
		fpos_t fpos;
		fd = fopen(argv[j + 1], "r");
		while (fgetpos(fd, &fpos), fread(XMLBuf, bufferSize, 1, fd) == 1)
		{
			if (!XML_Parse(parser, XMLBuf, bufferSize, 0)) {
				fprintf(stderr, "error '%s' at line %" XML_FMT_INT_MOD \
					"u character %" XML_FMT_INT_MOD "u\n",
					XML_ErrorString(XML_GetErrorCode(parser)),
					XML_GetCurrentLineNumber(parser),
		...
		fsetpos(fd, &fpos);
		bufferSizeLast = fread(XMLBuf, 1, bufferSize, fd);
		fclose(fd);
		if (!XML_Parse(parser, XMLBuf, bufferSizeLast, 1)) {
			fprintf(stderr, "error '%s' at line %" XML_FMT_INT_MOD \
				"u character %" XML_FMT_INT_MOD "u\n",
				XML_ErrorString(XML_GetErrorCode(parser)),
				XML_GetCurrentLineNumber(parser),
	...
	free(XMLBuf);
#endif
	printf("%d loops, with buffer size %d. Average time per loop: %.2f\n",
